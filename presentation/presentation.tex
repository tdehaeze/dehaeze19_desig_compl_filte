...
\label{sec:orgeac405f}
\begin{frame}[label={sec:org7acc6c9}]{Sensor Fusion Architecture - Noise Filtering}
\vspace{-1em}
\begin{columns}
...
\begin{frame}[label={sec:orgc2c6d2b}]{Sensor Fusion Architecture - Robustness}
\begin{columns}
...
\begin{frame}[label={sec:orgf43ebc6}]{Shaping of Complementary Filters using \(\mathcal{H}_\infty\) synthesis}
\vspace{-1em}
\begin{columns}
\begin{column}{0.5\columnwidth}
...
\begin{frame}[label={sec:org65cb237}]{Validation of the proposed synthesis method}
\begin{center}
\includegraphics[scale=1,width=0.85\linewidth]{figs/hinf_synthesis_results.pdf}
\end{center}
\end{frame}

\begin{frame}[label={sec:org8b0df25}]{Complementary Filters Used at LIGO - Specifications}
...
\begin{frame}[label={sec:org114463e}]{\(\mathcal{H}_\infty\) Synthesis - Comparison with LIGO's FIR filters}
\begin{columns}
