...
Thus, this sensor fusion architecture permits to filter the noise of both sensors without introducing any distortion in the physical quantity to measure, and the estimation error is described by eqref:eq:estimate_error.
...
The Power Spectral Density (PSD) of the estimation error $\Phi_{\delta x}$ is
