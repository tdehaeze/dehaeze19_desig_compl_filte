...
The super sensor dynamics eqref:eq:super_sensor_dyn_uncertainty is no longer equal to $1$ and it now dependent on the sensor dynamics uncertainty weights $w_i(s)$ as well as on the complementary filters $H_i(s)$.
...
In Fig. ref:fig:comp_fir_ligo_hinf, their bode plot is compared with the FIR filters of order 512 obtained in cite:hua05_low_ligo and found to be very close to each other.
