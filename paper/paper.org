...
The $\mathcal{H}_\infty$ synthesis objective applied on $P(s)$ is to design two stable filters $H_1(s)$ and $H_2(s)$ such that the $\mathcal{H}_\infty$ norm of the transfer function from $w$ to $[z_3,\ z_2, \ z_1]$ is less than one eqref:eq:hinf_syn_obj_three.
...
By choosing $H_3(s) \triangleq 1 - H_1(s) - H_2(s)$, the proposed $\mathcal{H}_\infty$ synthesis solves the design problem eqref:eq:hinf_problem_gen. \par
...
** Introduction                                                     :ignore:
In this section, the usefulness/performance of the proposed method is demonstrated using


- Explain the application.
- Explain that FIR filters have been designed instead of IIR

The goal is to design one high pass filter and one low pass filter
