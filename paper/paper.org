...
#+AUTHOR: @@latex:\thanks{\textsuperscript{$\dagger$} Email: {\tt\small thomas.dehaeze@esrf.fr}}@@
...
* Application: Design of Complementary Filters used for the LIGO Active Vibration Isolation System
<<sec:application_ligo>>
** Introduction                                                     :ignore:
The complementary filter synthesis method presented in section ref:sec:hinf_method is now applied for the design of challenging complementary filters used for the active isolation system at the LIGO.

The use of these complementary filters is described in cite:hua05_low_ligo,hua04_polyp_fir_compl_filter_contr_system.
The approach taken by Hua in cite:hua05_low_ligo for the design of such filter is to write the synthesis of complementary FIR filters as a convex optimization problem.
However, the implementation of high order FIR filter is complex as it requires a lot of computation power.

** Specifications
<<sec:ligo_specifications>>

The specification of the and summarized below:
...
However, their order should not be too high to limit both the complexity of the optimization problem and the order of the synthesize complementary filters.

- Add transfer functions used for synthesis? Or how they are designed at least.

Type I Chebyshev filter or order $20$.
